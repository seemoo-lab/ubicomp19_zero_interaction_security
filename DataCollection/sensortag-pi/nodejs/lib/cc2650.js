...
// For the audio profile see here: 
// http://processors.wiki.ti.com/index.php/BLESDK-2.2.x-CC2650RC_Developers_Guide#TI_Audio_Profile
var AUDIO_UUID                              = 'f000b00004514000b000000000000000';
var AUDIO_CONFIG_UUID                       = 'f000b00104514000b000000000000000'; 
var AUDIO_STREAM_UUID                       = 'f000b00204514000b000000000000000'; 

// Which params vary?: 1) number of files 2) number of flags -> both depend on the number of tags used !!!
var fs = require('fs');
var audioFileName = 'data/audio';
...
var CC2650SensorTag = function(peripheral) {
  ...
  this.onAudioChangeBinded       = this.onAudioChange.bind(this); // audio added
};
...
// Listener on 'AudioChange'
CC2650SensorTag.prototype.onAudioChange = function(data) {
  this.convertAudioData(data, function(audio) {
    this.emit('AudioChange', audio);
  }.bind(this));
};

// Append audio data to a file
CC2650SensorTag.prototype.convertAudioData = function(data, callback) {
  var audioData = Buffer.from(data);
  if(audioData.length == 1) {
	if(audioData.readUInt8(0) == 0) {
      callback(false);
	}
  } else {
    fs.appendFile(audioFileName, audioData, function (err) {
	  if(err) throw err;
	});
  }
  callback(true);
...
// Audio notifications
CC2650SensorTag.prototype.notifyAudioConfig = function(callback) {
  this.notifyCharacteristic(AUDIO_UUID, AUDIO_CONFIG_UUID, true, this.onAudioChangeBinded, callback);
};

CC2650SensorTag.prototype.unnotifyAudioConfig = function(callback) {
  this.notifyCharacteristic(AUDIO_UUID, AUDIO_CONFIG_UUID, false, this.onAudioChangeBinded, callback);
};

CC2650SensorTag.prototype.notifyAudioStream = function(callback) {
  this.notifyCharacteristic(AUDIO_UUID, AUDIO_STREAM_UUID, true, this.onAudioChangeBinded, callback);
};

CC2650SensorTag.prototype.unnotifyAudioStream = function(callback) {
  this.notifyCharacteristic(AUDIO_UUID, AUDIO_STREAM_UUID, false, this.onAudioChangeBinded, callback);
};
